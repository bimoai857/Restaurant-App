        ...
        <div class="max-w-7xl mx-auto  sm:px-6 lg:px-8">
           ...
           <div class="m-2 p-2 ">
            <div class="max-w-md mx-auto">
                <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="post" action="{{ route('admin.categories.update',$category->id) }} " enctype="multipart/form-data">
                 @csrf
                 @method('PUT')
                  <div class="mb-4">
                    ...
                    <input
                      ...
                      id="post-title" type="text" placeholder="Enter Category Name" name='name' value="{{ $category->name }}"
                    >
                    @error('name')
                  ...
                  <div class="mb-4">
                    ...
                    <div>
                        <img class="w-12 h-12" src="{{ Storage::url($category->image) }}"/>
                    </div>
                    <input
                      class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                      id="post-image" type="file" name="image" value="{{ $category->image }}"
                    >
                  ...
                  <div class="mb-4">
                    ...
                    <textarea
                      ...
                      id="body" placeholder="Enter Category Description" name="description">{{ $category->description }}</textarea>
                      @error('description')
                      <div class="text-sm text-red-700">{{ $message }}</div>
                      @enderror
                  </div>
